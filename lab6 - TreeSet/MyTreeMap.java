...
public class MyTreeMap<K extends Comparable<? super K>,V> extends AbstractMap<K,V>  {
    ...
    /** finds the value corresponding to searchKey
     * 
     * @param searchKey
     * @return value if searchKey is key
     * @return get() of left if searchKey is less than key
     * @return get() of right if searchKey is greater than key
     * @return null if the appropriate sub-tree is null 
     */
    private V get(K searchKey) {
    	int comp = searchKey.compareTo(key);
        if(comp == 0) return value;
        else if(comp < 0){
        	if(left.isEmpty()) return null;
        	else return left.get(searchKey);
        } else {
    ...
    /** Replaces the value of key if it already exists
     * Otherwise inserts a new tree with key and value
     * @param key, value
     * @return this.value if key is this.key
     * @return put of left or right if key is less than or greater than this.key
    ...
    public V put(K key, V value){
    	...
    	if(this.isEmpty()){
    		...
    		if(comp == 0){
        		prevval = this.value;
        		this.value = value;
        	} else if(comp < 0) prevval = left.put(key, value);
        	  else prevval = right.put(key, value);
    	}
    	
    	int diff = left.height - right.height;
    	if(diff < -1 || diff > 1) restructure(this);
    ...
    private void restructure(MyTreeMap<K, V> node) {
        ...
        if (node.left.height > node.right.height) {
            ...
            if (node.left.left.height > node.left.right.height) {
                ...
                c = node;
                t0 = node.left.left;
                t1 = node.left.right.left;
                t2 = node.left.right.right;
                t3 = node.right;
            ...
            if (node.right.right.height > node.right.left.height) {
                ...
                t0 = node.left;
                t1 = node.right.left;
                t2 = node.right.right.left;
                t3 = node.right.right.right;
    ...
    /**
     * Removes the node with key rkey and returns its value
     * 
     * @param rkey
     * 
     */
     ...
     * 
     * @return this if isLeaf() and findMin() of left otherwise
     */
    public MyTreeMap<K, V> findMin(){
     ...
     * 
     * @return this if isLeaf() and findMax() of right otherwise
     */
    public MyTreeMap<K, V> findMax(){
    	if(this.isLeaf()) return this;
    	else return right.findMin();
    }
