...
public class MyLinkedList<Type> extends AbstractList<Type> {
	...
	public void add(int index, Type data){
		...
		if(index > this.size || index < 0) throw new IndexOutOfBoundsException("You tried to put something at index " + index + " but the list is only " + this.size + " long");
		
		Node nextnode;
		Node newnode = new Node(data);
		if(index > this.size/2){
			nextnode = this.last;
			for(int i = 0; i < this.size-index; i++) nextnode = nextnode.getPrev();
		} else {
			nextnode = this.first;
			for(int i = 0; i <= index; i++) nextnode = nextnode.getNext();
		}
